...
async def buscar_cambios(
    ...
    for cambio in cambios:
        try:
            if cambio.Auditoria_Cambios.Tipo_Operacion == "INSERT":
                estado_actual = json.loads(cambio.Auditoria_Cambios.Datos_Nuevos)
            elif cambio.Auditoria_Cambios.Tipo_Operacion == "UPDATE":
                datos_nuevos = json.loads(cambio.Auditoria_Cambios.Datos_Nuevos)
            ...
            historial.append({
                ...
                "datos_anteriores": json.loads(cambio.Auditoria_Cambios.Datos_Anteriores) if cambio.Auditoria_Cambios.Datos_Anteriores else None,
                "datos_nuevos": json.loads(cambio.Auditoria_Cambios.Datos_Nuevos) if cambio.Auditoria_Cambios.Datos_Nuevos else None
            })
        except json.JSONDecodeError:
            historial.append({
                "fecha": cambio.Auditoria_Cambios.Fecha_Operacion,
    ...
    return {
        "registro": {
            ...
            "primer_cambio": historial[0]["fecha"] if historial else None,
            "ultimo_cambio": historial[-1]["fecha"] if historial else None,
...
async def get_estadisticas_auditoria(
    ...
    por_tabla = db.query(
        ...
        func.count(distinct(Auditoria_Cambios.ID_Registro)).label('registros_afectados'),
        func.count(distinct(Auditoria_Cambios.ID_Usuario)).label('usuarios_distintos')
    ).filter(
